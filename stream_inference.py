...
def process_segmentation(model_path, input_source, sock, video_port, results_sock, results_port, stream_name, imgsz=256, threshold=0.3, fps_target=30):
    ...
    is_udp = isinstance(input_source, int) == False and input_source.startswith("udp:")
    if not is_udp:
        cap = cv2.VideoCapture(input_source)
        fps_source = cap.get(cv2.CAP_PROP_FPS) or fps_target  
        frame_delay = 1.0 / fps_source
    else:
        frame_delay = 1.0 / fps_target  # fixed fps
    ...
    with open(RESULTS_CSV, mode='a', newline='') as file:
        ...
        while True:
            ...
            if is_udp:
                frame = receive_udp_stream(video_port)
                if frame is None:
                    continue
            else:
                ret, frame = cap.read()
